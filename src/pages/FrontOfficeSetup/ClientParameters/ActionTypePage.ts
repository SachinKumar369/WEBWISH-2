...
export class ActionTypePage extends BaseCrudPage {
  ...
  private readonly actionTypeLink = this.page.getByRole('link', { name: 'Action Type' });
  ...
  protected async openPage(): Promise<void> {
    // ✅ Restore hover (important for menu visibility)
    ...
    await this.elementActions.click(this.actionTypeLink, 'Action Type');
  }
  ...
  // ✅ Wrapper method (so your test does NOT change)
  async runActionTypeCreateDeleteFlow(): Promise<void> {
    const data = [
