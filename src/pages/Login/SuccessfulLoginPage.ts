...
/**
 * Page Object for TC_LGN_001 — Successful Login with Valid Credentials
 *
 ...
 */
export class SuccessfulLoginPage extends BasePage {
  ...
  async openLoginUrl(url?: string): Promise<void> {
    const loginUrl = url || process.env.BASE_URL || 'https://qc2webwish.prologicfirst.in/login/z6cQJcxmrEbhhFXqdoj64Q%3D%3D';
  ...
  // ──────────────────────────────────────────────
  // Convenience — full TC_LGN_001 flow
  // ──────────────────────────────────────────────

  /**
   * Execute the complete TC_LGN_001 flow:
   ...
   */
  async performSuccessfulLogin(userId: string, password: string): Promise<'Property Selection' | 'Dashboard'> {
    // Step 1
