...
import { CopyRateManager } from '../../src/pages/ManagerFunction/CopyRateManager';
...
test.describe('Manager Function - Rate Setup (capture room type codes)', () => {
  ...
  test('MGR_RATE_SETUP_001: Capture room type codes and apply rates', async ({
    page,
    context,
  }) => {
    ...
    // NEW
    const copyRateManager = new CopyRateManager(page);
    ...
    logger.info(
      'Step 1: Opening Rate Manager and capturing room type codes',
    );
    ...
    logger.info(
      'Step 6b: Copying rates and validating rack versus complimentary amounts',
    );
    ...
    // NEW
    await copyRateManager.copyRatesAndVerifyAgainstRackRate('100');
    ...
    logger.info(
      'Step 6b complete: Rates copied and verified successfully',
    );
