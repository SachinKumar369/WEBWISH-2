...
import { SectionNoListReportPage } from '../../../src/pages/Reports/ParameterReports/SectionNoListReportPage';
...
test.describe('Parameter Reports - Section No List DB Validation', () => {
  ...
  test('PRR_SECTION_NO_LIST_001: should validate section no list PDF data with DB data', async ({ browser }) => {
    ...
    try {
      const reportPage = new SectionNoListReportPage(session.page, session.context);

      await reportPage.openSectionNoListReport();

      const popup = await reportPage.openReportPopup();
      const queryText = "select * from prm_common_masters where property_id='webwe' and param_type='RSEC'";
      const validationResult = await ParameterReportValidationHelper.validatePdfAgainstDatabase<CommonMasterRow>(popup, {
        reportTitle: 'Parameter Reports - Section No List DB Validation',
        artifactBaseName: 'section-no-list-report',
        queryText,
      ...
      expect(
        missing,
        `PDF report mismatches found for section no list report rows: ${missing.map((item) => item.rowNumber).join(', ')}`
      ).toHaveLength(0);
    } finally {
